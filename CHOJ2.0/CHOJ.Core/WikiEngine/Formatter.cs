...
namespace CHOJ.WikiEngine {
	...
	public static class Formatter {
		...
		private static Regex extendedUp = new Regex(@"\{up\:.+?\}", RegexOptions.Compiled | RegexOptions.IgnoreCase);
		private static Regex specialTag = new Regex(@"\{(wikititle|wikiversion|mainurl|up|rsspage|themepath|clear|br|top|searchbox|pagecount)\}", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
		private static Regex cloud = new Regex(@"\{cloud\}", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
		private static Regex nspace = new Regex(@"\{namespace\}", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
		private static Regex nspaceList = new Regex(@"\{namespacelist\}", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
		...
		private static string extendedUpReplacement = "GetFile.aspx?Provider=$&File=";
		private static string extendedUpReplacementForAttachment = "GetFile.aspx?Provider=$&IsPageAttachment=1&Page=@&File=";
		...
		/// <param name="linkedPages">The linked pages.</param>
		...
		public static string Format(string raw) {
			...
			match = javascript.Match(sb.ToString());
			while(match.Success) {
			...
			while(match.Success) {
				if(!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end)) {
					...
					if(page == null) {
						// Normal file
						sb.Insert(match.Index, extendedUpReplacement.Replace("$", prov));
					}
					else {
						// Page attachment
						sb.Insert(match.Index,
							extendedUpReplacementForAttachment.Replace("$", prov).Replace("@", page));
					}
				}
				ComputeNoWiki(sb.ToString(), ref noWikiBegin, ref noWikiEnd);
...
//			}
		...
		private static string BuildEditSectionLink(int id, string page) {
			StringBuilder sb = new StringBuilder(100);
			sb.Append(@"<a href=""");
			//UrlTools.BuildUrl(sb, "Edit.aspx?Page=", HttpUtility.UrlEncode(page), "&amp;Section=", id.ToString());
